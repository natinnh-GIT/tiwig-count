...
const EMPTY = {
  brand: "", model: "", serial_number: "", type: "",
  magnification: "", objective_lens: "", tube_diameter: "",
  reticle: "", focal_plane: "", adjustment_units: "",
  max_elevation: "", max_windage: "",
  finish: "", condition: "",
  purchase_date: "", purchased_from: "", purchase_price: "", current_value: "",
  mounted_on_firearm_id: "", mounted_on_firearm_name: "",
  mount_info: "", notes: "", photo_url: "",
};
...
export default function OpticModal({ item, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      max_elevation: form.max_elevation ? Number(form.max_elevation) : undefined,
      max_windage: form.max_windage ? Number(form.max_windage) : undefined,
      modified_et: getETNow(),
    };
    if (!item?.id) payload.created_et = getETNow();
    if (item?.id) {
      await base44.entities.Optic.update(item.id, payload);
    } else {
      await base44.entities.Optic.create(payload);
    }
    setSaving(false);
    onSaved();
  };
    ...
    <div style={S.overlay}>
      <div style={S.header}>
        ...
        <button
          ...
          style={{ ...S.saveBtn, opacity: (saving || !form.brand || !form.model) ? 0.5 : 1 }}
        >
          {saving ? "Saving…" : "Save"}
        </button>
      </div>
      ...
      <div style={S.body}>
        ...
        <div style={S.grid2}>
          <div>
            ...
            <SuggestInput value={form.brand} onChange={v => set("brand", v)} suggestions={brandSuggestions} placeholder="Vortex, Leupold…" darkStyle />
          </div>
          <div>
            <label style={S.label}>Model *</label>
            <SuggestInput value={form.model} onChange={v => set("model", v)} suggestions={modelSuggestions} placeholder="Razor HD, Mark 5…" darkStyle />
          </div>
        </div>
        ...
        <div style={S.grid2}>
          <div>
            ...
            <select value={form.type} onChange={e => set("type", e.target.value)} style={S.select}>
              ...
              {["Scope", "LPVO", "Red Dot", "Holographic", "Prism", "Binocular", "Monocular", "Rangefinder", "Night Vision", "Thermal", "Other"].map(t => (
        ...
        <div style={S.grid2}>
          ...
          <div>
            ...
            <SuggestInput value={form.finish} onChange={v => set("finish", v)} suggestions={finishSuggestions} placeholder="Matte Black, FDE…" darkStyle />
          </div>
        </div>
        ...
        <div style={S.grid2}>
          <div>
            <label style={S.label}>Magnification</label>
            <input value={form.magnification} onChange={e => set("magnification", e.target.value)} placeholder="5-25x, 1-6x, 4x…" style={S.input} />
          </div>
          <div>
            <label style={S.label}>Objective Lens</label>
            <input value={form.objective_lens} onChange={e => set("objective_lens", e.target.value)} placeholder="50mm, 44mm…" style={S.input} />
          </div>
        </div>

        <div style={S.grid2}>
          <div>
            <label style={S.label}>Tube Diameter</label>
            <select value={form.tube_diameter} onChange={e => set("tube_diameter", e.target.value)} style={S.select}>
              <option value="">— Select —</option>
              {["1 inch", "30mm", "34mm", "35mm", "other"].map(t => (
                <option key={t} value={t}>{t}</option>
              ))}
            </select>
          </div>
          <div>
            <label style={S.label}>Focal Plane</label>
            <select value={form.focal_plane} onChange={e => set("focal_plane", e.target.value)} style={S.select}>
              <option value="">— Select —</option>
        ...
        <div style={S.section}>
          <label style={S.label}>Reticle</label>
          <SuggestInput value={form.reticle} onChange={v => set("reticle", v)} suggestions={reticleSuggestions} placeholder="EBR-2C, Horus H59, BDC…" darkStyle />
        </div>

        <div style={S.grid2}>
          <div>
            <label style={S.label}>Adjustment Units</label>
            <select value={form.adjustment_units} onChange={e => set("adjustment_units", e.target.value)} style={S.select}>
              <option value="">— Select —</option>
        ...
        <div style={S.grid2}>
          <div>
            <label style={S.label}>Max Elevation {form.adjustment_units ? `(${form.adjustment_units})` : ""}</label>
            <input type="number" value={form.max_elevation} onChange={e => set("max_elevation", e.target.value)} placeholder="0" style={S.input} />
          </div>
          <div>
            <label style={S.label}>Max Windage {form.adjustment_units ? `(${form.adjustment_units})` : ""}</label>
            <input type="number" value={form.max_windage} onChange={e => set("max_windage", e.target.value)} placeholder="0" style={S.input} />
          </div>
        </div>
        ...
        <div style={S.section}>
          <label style={S.label}>Mounted On Firearm</label>
          <select
            value={form.mounted_on_firearm_id}
            onChange={e => handleFirearmChange(e.target.value)}
            style={S.select}
          >
        ...
        <div style={S.section}>
          <label style={S.label}>Mount / Rings Info</label>
          <input
            value={form.mount_info || form.rings_mounts || ""}
            onChange={e => set("mount_info", e.target.value)}
            placeholder="Badger Ordnance 30mm, 20 MOA base…"
            style={S.input}
          />
        </div>
        ...
        <div style={S.section}>
          <textarea
            ...
            placeholder="Zero data, maintenance log, accessories…"
            rows={4}
